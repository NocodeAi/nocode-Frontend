...
function MaterialReciept(props) {
  let data = props?.data;
  data.type = "materialReciept";
  ...
  const handleClick = (event) => {
    multiple
      ...
      : handleSelectNode(event, data?.id, props?.elements, props?.setElements);
  };
  const style = {
    color: "#333",
      ...
      <Tooltip
        ...
        tooltip={
          <div style={{ display: "block" }}>
            <constants.AiOutlineSetting
              onClick={onClickSetting}
              style={iconStyle}
            />
            <constants.AiOutlineDelete
...
export default MaterialReciept;
